...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
for pplID = 1:numPeople
    parents = pedigree.parents(pplID,:);
    if all(parents) % all non-zero
        % first copy factor if parent is present
        factorList(pplID) = childCopyGivenParentalsFactor(numAlleles,pplID, parents(1), parents(1)+numPeople);
        % second copy factor if parent is present
        factorList(pplID +numPeople) = childCopyGivenParentalsFactor(numAlleles,pplID+numPeople, parents(2), parents(2)+numPeople);
    else
        % first copy factor
        factorList(pplID) = childCopyGivenFreqsFactor(alleleFreqs,pplID);
        % second copy factor
        factorList(pplID +numPeople) = childCopyGivenFreqsFactor(alleleFreqs,numPeople+pplID);
    end
    % phenotype factor
    factorList(pplID +2*numPeople) = phenotypeGivenCopiesFactor(alphaList,numAlleles,pplID,pplID+numPeople,pplID+2*numPeople);
end
