...
% Vector of number of alleles per gene
...
assignment = IndexToAssignment(1:length(phenotypeFactor.val),phenotypeFactor.card);
for i = 1:size(assignment,1)
    phenotype = assignment(i,1);
    expSum = 0;
    for j = 1:numGenes
        expSum = expSum + alleleWeights{j}(assignment(i,j+1)) ...
                        + alleleWeights{j}(assignment(i,j+1+numGenes));
    end
    if phenotype == 1
        phenotypeFactor.val(i) = computeSigmoid(expSum);
    else
        phenotypeFactor.val(i) = 1 - computeSigmoid(expSum);
    end
end
