...
allSimilarities = ones(length(allFactors),1);
for i = 1:length(allFactors)
    allSimilarities(i) = allFactors(i).val(1);
    % similarity is reused every time Ci=Cj, so we take Ci=Cj=1 
end
% select top similarity factors
[B,IX] = sort(allSimilarities,'descend');
factors = allFactors(IX(1:F));
