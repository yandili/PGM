...
numGeno = size(genotypesToAlleles,1);
genotypeFactor.card = [numGeno, numGeno, numGeno];

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
for p1 = 1:numGeno
    for p2 = p1:numGeno
        genoFreqs = zeros(1,numGeno);
        for a1 = genotypesToAlleles(p1,:)
            for a2 = genotypesToAlleles(p2,:)
                genoFreqs(allelesToGenotypes(a1,a2)) = 0.25 + genoFreqs(allelesToGenotypes(a1,a2));
            end
        end
        % assignment of genoChild, genoParent1 and genoParent2
        A = [1:numGeno, 1:numGeno;
            ones(1,numGeno)*p1, ones(1,numGeno)*p2;
            ones(1,numGeno)*p2, ones(1,numGeno)*p1]';
        % corresponding probability values
        v = [genoFreqs, genoFreqs];
