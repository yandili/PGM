...
imageSimilarity = ImageSimilarity(images(i).img, images(j).img);
for idx = AssignmentToIndex([1:K;1:K]', factor.card)
    % assignments when Ci==Cj
    factor.val(idx) = imageSimilarity;
end
